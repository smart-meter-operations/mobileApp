...
export class DataService {
  ...
  static async getDashboardStats() {
    ...
    const installationStats = {
      ...
      completed: installations.filter(item => item.status === STATUS.COMPLETED).length,
      pending: installations.filter(item => item.status === STATUS.PENDING).length,
    };
    
    const surveyStats = {
      total: surveys.length,
      completed: surveys.filter(item => item.status === STATUS.COMPLETED).length,
