...
const FormDropdown = ({
  ...
  <View style={[styles.inputContainer, style]}>
    ...
    />
    {/** optional inline error intentionally omitted for FormDropdown */}
  </View>
);
      ...
      <Controller
        ...
        render={({ field: { onChange, value }, fieldState: { error } }) => (
          <TouchableOpacity
            ...
            onPress={async () => {
              // If in completed/read-only mode, show image instead of opening camera
              if (currentStatus === 'Completed') {
                Alert.alert('View Image', 'Image viewing is not available in this version.');
                return;
              }
              ...
              try {
                ...
                if (!result.canceled) {
                  ...
                  if (!asset) {
                    ...
                    Alert.alert('Capture Error', 'No image data returned from camera. Please try again.');
                    return;
                  }
                  ...
                  if (asset.base64) {
                    ...
                    Alert.alert('Capture Error', 'Invalid image data. Please try again.');
                  }
                }
              } catch (e) {
                ...
                Alert.alert('Capture Error', `Failed to capture image: ${e?.message || 'Unknown error'}`);
              }
            }}
          >
            <Ionicons
...
export default function ConsumerIndexingFormScreen({ navigation, route }) {
  ...
  const loadConsumerData = useCallback(async () => {
    try {
      ...
      if (consumerData) {
        ...
        // Handle photo fields
        if (consumerData.pole_photo) setValue('polePhoto', 'photo_captured');
        if (consumerData.old_meter_photo)
          setValue('oldMeterPhoto', 'photo_captured');
        if (consumerData.old_meter_kwh_photo)
          setValue('oldMeterKWHPhoto', 'photo_captured');
        if (consumerData.house_photo) setValue('housePhoto', 'photo_captured');

        // Handle SIM data fields - Load from database for completed records
        setValue('sim1NetworkProvider', consumerData.sim1NetworkProvider || '');
        setValue('sim1SignalStrength', consumerData.sim1SignalStrength || '');
        setValue('sim1SignalLevel', consumerData.sim1SignalLevel || '');
        setValue('sim1SignalType', consumerData.sim1SignalType || '');
        setValue('sim1Category', consumerData.sim1Category || '');
        setValue('sim1RSSI', consumerData.sim1RSSI || '');
        setValue('sim1RSRP', consumerData.sim1RSRP || '');
        setValue('sim1SNR', consumerData.sim1SNR || '');
        setValue('sim1CellId', consumerData.sim1CellId || '');

        setValue('sim2NetworkProvider', consumerData.sim2NetworkProvider || '');
        setValue('sim2SignalStrength', consumerData.sim2SignalStrength || '');
        setValue('sim2SignalLevel', consumerData.sim2SignalLevel || '');
        setValue('sim2SignalType', consumerData.sim2SignalType || '');
        setValue('sim2Category', consumerData.sim2Category || '');
        setValue('sim2RSSI', consumerData.sim2RSSI || '');
        setValue('sim2RSRP', consumerData.sim2RSRP || '');
        setValue('sim2SNR', consumerData.sim2SNR || '');
        setValue('sim2CellId', consumerData.sim2CellId || '');
      } else {
  ...
  const handlePhotoCapture = async (name, imageSource) => {
    if (!consumerId) {
      Alert.alert('Error', 'Consumer ID is not available. Cannot save image.');
      return;
    }
    try {
      if (!imageSource) {
      ...
      // Get current GPS location and network info when image is captured
      ...
      let base64Data = '';
      // Case 1: object with base64 (from ImagePicker when base64:true)
      if (typeof imageSource === 'object' && imageSource.base64) {
        base64Data = String(imageSource.base64);
        console.log('handlePhotoCapture: received base64 directly from picker', { name, length: base64Data.length });
      }
      // Case 2: string file URI
      else if (typeof imageSource === 'string' && imageSource.startsWith('file://')) {
        // Prefer direct file read for file:// URIs to avoid manipulator dependency
        try {
          const FileSystem = await import('expo-file-system');
          console.log('handlePhotoCapture: reading base64 via FileSystem', { name });
          base64Data = await FileSystem.readAsStringAsync(imageSource, { encoding: FileSystem.EncodingType.Base64 });
        } catch (fsErr) {
          console.error('FileSystem read failed, attempting ImageManipulator compress/base64', fsErr?.message);
          try {
            ...
            console.error('ImageManipulator also failed for file:// URI', manipErr?.message);
          }
        }
      }
      // Case 3: assume raw base64 string
        ...
        console.log('handlePhotoCapture: received raw base64 string', { name, length: base64Data.length });
      }
      // Case 4: non-file URI (content:// etc.)
      else {
        // For content:// or others, try ImageManipulator first
        console.log('handlePhotoCapture: non-file URI, trying ImageManipulator', { name });
        try {
          const ImageManipulator = await import('expo-image-manipulator');
          const manipulated = await ImageManipulator.manipulateAsync(
            imageSource,
          ...
          console.error('ImageManipulator failed for non-file URI; cannot fallback to FileSystem', manipErr?.message);
        }
      }

      // Keep form state light to avoid memory spikes
      setValue(name, 'photo_captured');

      if (base64Data && typeof base64Data === 'string') {
        try {
          console.log('Saving image to database...');
          // Save image with GPS coordinates and network info
          ...
          if (!result.success) {
            console.error('saveConsumerImage failed:', result);
            Alert.alert('Error', `Failed to save image to database: ${result.error || 'Unknown error'}`);
          } else {
            const gpsStatus = gpsCoordinates ? 'with GPS coordinates' : 'without GPS coordinates';
            const networkStatus = networkInfo ? 'with network info' : 'without network info';
            console.log(`Image saved to DB for ${name} ${gpsStatus} ${networkStatus}`);
            
            // Update form fields with captured GPS coordinates
            if (gpsCoordinates && gpsCoordinates.latitude && gpsCoordinates.longitude) {
              ...
              console.log('Updated form GPS fields:', {
                latitude: gpsCoordinates.latitude,
                longitude: gpsCoordinates.longitude
              });
            }
            
            // Fill SIM1 fields with dummy data instead of real network info
            setValue('sim1NetworkProvider', 'Jio');
            setValue('sim1SignalStrength', '-85 dBm');
            setValue('sim1SignalLevel', 'Good');
            setValue('sim1SignalType', '4G');
            setValue('sim1Category', 'LTE');
            setValue('sim1RSSI', '-85');
            setValue('sim1RSRP', '-105');
            setValue('sim1SNR', '15');
            setValue('sim1CellId', 'CID001');
            console.log('Updated form SIM1 fields with dummy data');
            
            // Removed success alert as requested
          }
        } catch (dbError) {
          console.error('Database save error:', dbError);
          Alert.alert('Error', `Database error: ${dbError.message || 'Unknown error'}`);
        }
      } else {
        console.error('No base64 generated from URI. Not saving to DB.', { name, uri: imageSource });
        Alert.alert('Error', 'Could not process captured image.');
      }
    } catch (e) {
  ...
  // Helper: build mapped payload with exact server keys
  const buildMappedPayload = (rawValues) => {
    ...
    // Helper function to format phone numbers with +91 prefix
    ...
    const mapped = {
      ...
      sim1_network_service_provider: mapVal(withoutMeta.sim1NetworkProvider),
      sim1_signal_strength: mapVal(withoutMeta.sim1SignalStrength),
      sim1_signal_level: mapVal(withoutMeta.sim1SignalLevel),
      sim1_signal_type: mapVal(withoutMeta.sim1SignalType),
      sim1_category: mapVal(withoutMeta.sim1Category),
      sim1_rssi: mapVal(withoutMeta.sim1RSSI),
      sim1_rsrp: mapVal(withoutMeta.sim1RSRP),
      sim1_snr: mapVal(withoutMeta.sim1SNR),
      sim1_cell_id: mapVal(withoutMeta.sim1CellId),
      sim2_network_service_provider: mapVal(withoutMeta.sim2NetworkProvider),
      sim2_signal_strength: mapVal(withoutMeta.sim2SignalStrength),
      sim2_signal_level: mapVal(withoutMeta.sim2SignalLevel),
      sim2_signal_type: mapVal(withoutMeta.sim2SignalType),
      sim2_category: mapVal(withoutMeta.sim2Category),
      sim2_rssi: mapVal(withoutMeta.sim2RSSI),
      sim2_rsrp: mapVal(withoutMeta.sim2RSRP),
      sim2_snr: mapVal(withoutMeta.sim2SNR),
      sim2_cell_id: mapVal(withoutMeta.sim2CellId),
      ci_remarks: mapVal(withoutMeta.ciRemarks),
      je_name: mapVal(withoutMeta.jeName),
      pole_photo: mapVal(withoutMeta.pole_photo),
      old_meter_photo: mapVal(withoutMeta.old_meter_photo),
      old_meter_kwh_reading_photo: mapVal(withoutMeta.old_meter_kwh_photo),
      house_photo: mapVal(withoutMeta.house_photo),
      // GPS coordinates
  ...
  const onSubmit = async (data) => {
    try {
      // Enhanced validation that checks form field values
      ...
      await execute(async () => {
        ...
        const docName = formValues.consumerId || route.params?.consumerId || 'CI-2025-006';
        const payload = buildMappedPayload(formValues);

        // Try immediate PUT to Frappe with enhanced logging
        try {
          const submitTimestamp = new Date().toISOString();
          console.log(`🎯 [${submitTimestamp}] FORM SUBMISSION START - Consumer Survey`);
          console.log(`📋 DocName: ${docName}`);
          console.log(`🔑 Payload Keys (${Object.keys(payload).length}):`, Object.keys(payload));

          // Log GPS and SIM data specifically for form submission
          const gpsData = {
            LATITUDE_M: payload.LATITUDE_M,
            LONGITUDE_M: payload.LONGITUDE_M
          };
          const simData = Object.fromEntries(
            Object.entries(payload).filter(([key]) => key.startsWith('SIM1_'))
          );
          console.log(`📍 Form GPS Data:`, gpsData);
          console.log(`📶 Form SIM1 Data:`, simData);

          try {
            console.log(`📤 Form Submit Payload:`, JSON.stringify(payload, null, 2));
          } catch (e) {
            console.log(`📤 Form Submit Payload (stringified):`, payload);
          }

          console.log(`⏳ Making form submit API call...`);
          const apiRes = await ApiService.updateConsumerSurveyAbsolute({ docName, payload });

          const responseTimestamp = new Date().toISOString();
          console.log(`✅ [${responseTimestamp}] FORM SUBMISSION RESPONSE RECEIVED`);
          try {
            console.log(`📥 Form Submit Response:`, JSON.stringify(apiRes, null, 2));
          } catch (e) {
            console.log(`📥 Form Submit Response (raw):`, apiRes);
          }

          if (apiRes.success) {
            console.log(`🎉 [${new Date().toISOString()}] FORM SUBMISSION SUCCESS - Consumer Survey Completed`);
            // Option A: mark local record completed
            await DatabaseService.updateConsumerIndexingStatus(docName, 'Completed');
          ...
          console.error('Online submission failed (screen submit):', error?.message, error?.stack, error);
        }

        // Fallback to offline save
        const result = await saveConsumerIndexingData(data, 'ToSync', false);
        if (result.success) {
          Alert.alert('Saved Offline', 'No network or server error. Data saved and will sync later.', [
            {
              text: 'OK',
              onPress: () => navigation.goBack(),
            },
      ...
      Alert.alert(
        'Error',
        'Failed to save consumer indexing data. Please try again.'
      );
    }
  };
  ...
  const handleSyncNow = async () => {
    try {
      await execute(async () => {
        const values = control._formValues;
        const docName = values.consumerId || route.params?.consumerId;
        if (!docName) {
          Alert.alert('Error', 'Missing Consumer ID to sync.');
          return;
        }
        // Build mapped payload
        const payload = buildMappedPayload(values);

        console.log('Sync Now Consumer Survey (screen):', { docName, keys: Object.keys(payload) });
        try { console.log('Sync Now payload (exact JSON):', JSON.stringify(payload, null, 2)); } catch {}
        const apiRes = await ApiService.updateConsumerSurveyAbsolute({ docName, payload });
        try { console.log('Sync Now API response:', JSON.stringify(apiRes, null, 2)); } catch { console.log('Sync Now API response (raw):', apiRes); }
        if (apiRes.success) {
          await DatabaseService.updateConsumerIndexingStatus(docName, 'Completed');
          ...
          console.error('Sync Now failed:', apiRes);
          Alert.alert('Sync Failed', apiRes.message || 'Server unreachable. Please try again later.');
        }
      });
    } catch (error) {
  ...
  const saveConsumerIndexingData = async (
    ...
    try {
      ...
      const dbData = {
        ...
        conditionOfInstalledServiceCable: data.conditionOfInstalledServiceCable,
        ...
        oldMeterReadingMD: data.oldMeterReadingMD,
        remarks: data.ciRemarks,
        jeName: data.jeName,
        // SIM1 data
        sim1NetworkProvider: data.sim1NetworkProvider,
        sim1SignalStrength: data.sim1SignalStrength,
        sim1SignalLevel: data.sim1SignalLevel,
        sim1SignalType: data.sim1SignalType,
        sim1Category: data.sim1Category,
        sim1RSSI: data.sim1RSSI,
        sim1RSRP: data.sim1RSRP,
        sim1SNR: data.sim1SNR,
        sim1CellId: data.sim1CellId,
        // SIM2 data
        sim2NetworkProvider: data.sim2NetworkProvider,
        sim2SignalStrength: data.sim2SignalStrength,
        sim2SignalLevel: data.sim2SignalLevel,
        sim2SignalType: data.sim2SignalType,
        sim2Category: data.sim2Category,
        sim2RSSI: data.sim2RSSI,
        sim2RSRP: data.sim2RSRP,
        sim2SNR: data.sim2SNR,
        sim2CellId: data.sim2CellId,
        // GPS coordinates
        latitude: data.latitude,
        longitude: data.longitude,
        // Photo data
        pole_photo: data.pole_photo,
        old_meter_photo: data.old_meter_photo,
        old_meter_kwh_photo: data.old_meter_kwh_photo,
            ...
            <View style={styles.formContainer}>
              ...
              >
                ...
                <PhotoCapture
                  ...
                  onPhotoCapture={(base64) => handlePhotoCapture('old_meter_photo', base64)}
                  currentStatus={currentStatus}
                />
                ...
                <PhotoCapture
                  ...
                  onPhotoCapture={(base64) => handlePhotoCapture('old_meter_kwh_photo', base64)}
                  currentStatus={currentStatus}
                />
